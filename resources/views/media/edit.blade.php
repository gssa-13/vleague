            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('media.update', $media) }}" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="media_type_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                            @foreach ($mediaTypes as $type)
                                <option value="{{ $type->id }}" {{ old('media_type_id', $media->media_type_id) == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="name" value="{{ old('name', $media->name) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
