                ...
                <form method="POST" action="{{ route('payments.update', $payment) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="competition_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                            ...
                            @foreach ($competitions as $competition)
                                <option value="{{ $competition->id }}" {{ old('competition_id', $payment->competition_id) == $competition->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="concept" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                            @foreach (App\Enums\PaymentConcept::cases() as $concept)
                                <option value="{{ $concept->value }}" {{ old('concept', $payment->concept->value) === $concept->value ? 'selected' : '' }}>{{ $concept->label() }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            <input type="number" step="0.01" name="amount" value="{{ old('amount', $payment->amount) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                        ...
                        <div>
                            ...
                            <input type="number" step="0.01" name="paid_amount" value="{{ old('paid_amount', $payment->paid_amount) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
