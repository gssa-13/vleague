            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <form method="POST" action="{{ route('divisions.update', $division) }}">
                        ...
                        <select name="tournament_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                            @foreach ($tournaments as $tournament)
                                <option value="{{ $tournament->id }}"
                                    {{ old('tournament_id', $division->tournament_id) == $tournament->id ? 'selected' : '' }}>
                                    {{ $tournament->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="day" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                            @foreach (App\Enums\DivisionDay::cases() as $day)
                                <option value="{{ $day->value }}"
                                    {{ old('day', $division->day->value) === $day->value ? 'selected' : '' }}>
                                    {{ $day->label() }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="field_number"
                               value="{{ old('field_number', $division->field_number) }}" min="1"
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="group_letter"
                               value="{{ old('group_letter', $division->group_letter) }}"
                               maxlength="1"
                               class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
