<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Edit Payroll') }}</h2>
    </x-slot>
    <div class="py-12">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('payrolls.update', $payroll) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="venue_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                            @foreach ($venues as $venue)
                                <option value="{{ $venue->id }}" {{ old('venue_id', $payroll->venue_id) == $venue->id ? 'selected' : '' }}>{{ $venue->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="period" value="{{ old('period', $payroll->period) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
